...
Combo sheets can be exported as images or as files that can be later imported.
...
If you just want to use the tool please consult the `Releases` tab on the Github page and install the desired version compatible with your OS.

If you want to to execute the python code directly install the required dependencies with:
...
If you downloaded the project under the release tab just start it like any other normal program.

If you downloaded the codebase you can start the application with:

```bash
python app.py
```

Usage instructions such as shortcuts are written under the help tab available while editing a combo sheet.
Application may become unresponsive when importing ydk that contains cards not in cache.
